...
class PygameSOM:
    ...
    def cartesian_to_pixel_point(self, cartesian_point):
        ...
        return int(x_half_screen_size + x * x_units_pixel_size), int(y_half_screen_size + y * y_units_pixel_size)

    def update_screen(self, networksom):
        surface = pygame.Surface((self.width, self.height), pygame.SRCALPHA)

        for point in networksom.trainX:
            pixel_point = self.cartesian_to_pixel_point(point)
            pygame.draw.rect(surface, (255, 0, 0), (pixel_point[0], pixel_point[1], 10, 10))

        for neuron in networksom.neurons:
            neuron_point = self.cartesian_to_pixel_point(neuron) 
            pygame.draw.circle(surface, (0, 0, 255), neuron_point, 5)
